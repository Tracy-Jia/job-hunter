...
>
> 求职市场上99%的工具都在做同一件蠢事：帮你一天投500份。这相当于把一份简历复印500次然后随机扔进不同的窗户——你的确很忙，但忙的事毫无价值。**HR在看到"期待进一步沟通"的第三秒就知道你是海投。** 这个工具反着来：每天只处理20-30个岗位，但每一个都读过JD、做过匹配、招呼语是专门写的。不是因为我们道德高尚——而是因为这样做确实更有效。

全链路：扫描 → 筛选 → 读JD → AI匹配 → 生成招呼语 → 自动发送 → 每日复盘。基于 [Claude Code](https://claude.com/claude-code) 的 skill，通过 CDP（Chrome DevTools Protocol）操控本地浏览器。**纯本地运行，不上传任何数据到第三方。**

> **平台覆盖**：核心流水线已在一线招聘平台上经过数十轮实战验证。51job / 猎聘 / 鱼泡直聘的适配脚本保留在仓库中供社区参考和贡献，尚未经过同等强度验证。

## ⚠️ 免责声明

**本项目仅供学习研究使用。** 使用者应自行评估目标平台的服务条款（Terms of Service），确保使用方式符合平台规定。自动化操作可能违反部分招聘平台的用户协议，由此产生的账号风险由使用者自行承担。

## 这个工具不做什么——以及为什么不做

**不做海投。** 海投的首要问题是激励机制错位——工具奖励"发送数量"，不奖励"回复质量"。你把100条模板消息发出去，收到2条回复，工具告诉你"今日已投100份，效率提升200%"。这是用行动量替代结果量，跟前苏联用吨位考核炼钢厂是一回事——你确实得到了很多吨钢，但全是没法用的废铁。

**不做一键全自动。** 发送前有逐条确认环节。理由很简单：招呼语是你给HR的第一印象，而这个印象决定了对方要不要花15秒打开你的简历。让机器不经人类确认就替你说话——这不叫自动化，这叫放弃责任。

## 它到底做什么

| 环节 | 做什么 | 为什么这比大多数方案合理 |
|------|--------|-------------|
| **扫描** | 多关键词批量扫卡片，自动去重 | 12个词 × 每词10条 → 去重后 ~90条，3分钟。你不应该花时间翻列表，你应该花时间读JD |
...
5段式框架是经过数十轮迭代验证出来的——不是在纸上设计好然后实现的，是在实战中被HR已读不回打脸后一遍遍改出来的。详见 `greeting_guide.md`（这是整个仓库里最值得读的文件）。
...
**角色语气切换**：薪酬绩效岗用专业硬核语气（"做过两条产品线的差异化提成方案"），HRBP用业务视角（"之前支持过150人的销售团队"），总助/办主任用干练执行力，HR全盘全面但不泛。
