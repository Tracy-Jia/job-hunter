...
> **适用场景**：面向国内招聘市场。核心流水线在一线招聘平台上经过数十轮实战验证，体感最好的平台因合规考虑不在此处具名。如果你不在国内求职，这个工具没用。
...
| 环节 | 做什么 | 为什么这比大多数方案合理 |
|------|--------|-------------|
| **扫描** | 多关键词批量扫卡片，自动去重 | 根据求职方向拆成多个搜索词（不同title变体、相邻职能），每个词扫若干条，去重后得到一个不重复的岗位池。拆几个词取决于求职者的方向有多宽 |
| **筛选** | 由求职者定义自己的排除规则，系统自动执行 | 你想避开什么？保险/教培/外包/单休/远距离/英语口语岗/已投过的公司……所有你不想浪费时间看的，写进配置里一次性筛掉。规则你做主，机器只负责不遗漏、不手滑 |
| **读JD** | CDP逐个打开详情页，绕过字体加密提取明文薪资 | 部分平台把薪资数字用自定义字体加密，普通爬虫只能抓到乱码。直接从页面script标签里拿原始数据 |
| **招呼语** | 5段式框架，基于JD逐条定制，按岗位类型切换语气 | 整套流水线的护城河。每条招呼语都经过"提取JD需求 → 匹配简历证据 → 选钩子 → 生成 → 质量自检"。不再千篇一律 |
