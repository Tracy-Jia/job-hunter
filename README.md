...
| **筛选** | 由求职者定义自己的排除规则，系统自动执行 | 想避开什么？保险/教培/外包/单休/远距离/英语口语岗/已投过的公司……所有不想浪费时间看的，写进配置一次性筛掉。规则自己做主，机器只负责不遗漏 |
| **读JD** | CDP逐个打开详情页，绕过字体加密提取明文薪资 | 部分平台把薪资数字用自定义字体加密，普通爬虫只能抓到乱码。我们直接从页面script标签里拿原始数据 |
| **招呼语** | 5段式框架，基于JD逐条定制，按岗位类型切换语气 | 这是整套流水线的护城河。每条招呼语都经过"提取JD需求 → 匹配简历证据 → 选钩子 → 生成 → 质量自检"。不再只是千篇一律的"您好，我非常希望能得到这个岗位的面试机会..." |
